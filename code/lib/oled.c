...
uint8_t WriteCmd(unsigned char I2C_Command)//写命令
 {
...
uint8_t OLED_Init(void)
{
	...
	if(WriteCmd(0xAE)!=HAL_OK) return 1; //display off
	...
	switch(TextSize)
	{
		...
		case 2:
		{
			while(ch[j] != '\0')
			{
				...
				OLED_SetPos(118-x,7-y);
				for(i=0;i<8;i++){
					c = F8X16[(ch[j] - 32)*16+7-i];
					d = (c>>7)+
					(((c>>6)&1)<<1)+
					(((c>>5)&1)<<2)+
					(((c>>4)&1)<<3)+
				...
				OLED_SetPos(118-x,7-y-1);
				for(i=0;i<8;i++){
					c = F8X16[(ch[j] - 32)*16+7-i+8];
					d = (c>>7)+
					(((c>>6)&1)<<1)+
					(((c>>5)&1)<<2)+
					(((c>>4)&1)<<3)+
