...
void WritetoSD(BYTE write_buff[],uint8_t bufSize)
{
	...
	if(res == 1)
	{
		sprintf(buf,"SD卡初始化失败! \r\n");		
	}
	else
	{
		sprintf(buf,"SD卡初始化成功！ \r\n");		
	}
...
//		test_sd =1;				//用于测试格式化
		...
		if(res == FR_OK)
		{
			...
			if(res == FR_OK)
			{
				sprintf(buf,"SD卡已经成功挂载，可以进进行文件写入测试!\r\n");
			}	
		}
		else
		{
	...
	{
		sprintf(buf,"卡存储已满，写入失败!\r\n");		
	}
	
	f_lseek(&file, f_size(&file));//确保写词写入不会覆盖之前的数据
	if(res == FR_OK)
	{
		sprintf(buf,"打开成功/创建文件成功！ \r\n");		
...
void Get_SDCard_Capacity(void)
{
	...
	if (res != FR_OK)
	{
		sprintf(buf,"FileSystem Mounted Failed (%d)\r\n", result);
	}
	...
	{
		sprintf(buf,"Get SDCard Capacity Failed (%d)\r\n", result);
	}		
} 
